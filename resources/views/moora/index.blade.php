...
<div class="flex items-center gap-2.5 bg-purple-50/80 border border-purple-200/80 px-3.5 py-2.5 mb-5 rounded-xl text-xs text-gray-700 shadow-2xs">
    <i class="bi bi-info-circle-fill text-brand-purple text-sm flex-shrink-0"></i>
    <p class="leading-tight">
        <strong>Peringkat #1</strong> adalah prioritas perbaikan tertinggi (paling mendesak) metode MOORA.
    </p>
</div>
...
<div class="grid grid-cols-2 gap-2.5 sm:gap-3.5 md:hidden">
        ...
        <div class="bg-white rounded-2xl border transition-all duration-200 shadow-xs p-3.5 sm:p-4 flex flex-col justify-between space-y-3 h-full {{ $isRank1 ? 'border-amber-300 ring-2 ring-amber-200/60 bg-amber-50/25' : ($isTop3 ? 'border-purple-200/90' : 'border-gray-200') }}">
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between gap-1.5">
                    @if ($isRank1)
                        <span class="w-8 h-8 rounded-xl bg-amber-400 text-amber-950 font-black text-xs flex items-center justify-center shadow-xs ring-1 ring-amber-300 flex-shrink-0">
                            #1
                        </span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-900 border border-amber-300 truncate">
                            <i class="bi bi-star-fill text-amber-500 mr-1 text-[9px]"></i> Utama
                        </span>
                    @elseif ($isRank2)
                        <span class="w-8 h-8 rounded-xl bg-slate-200 text-slate-800 font-black text-xs flex items-center justify-center shadow-xs border border-slate-300 flex-shrink-0">
                            #2
                        </span>
                    ...
                    @elseif ($isRank3)
                        <span class="w-8 h-8 rounded-xl bg-amber-100 text-amber-900 font-black text-xs flex items-center justify-center shadow-xs border border-amber-200 flex-shrink-0">
                            #3
                        </span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-50 text-red-700 border border-red-200">
                            Mendesak
                    ...
                    @else
                        <span class="w-8 h-8 rounded-xl bg-gray-100 text-gray-700 font-bold text-xs flex items-center justify-center border border-gray-200 flex-shrink-0">
                            #{{ $row['rank'] }}
                        </span>
                        @if ($row['rank'] <= 7)
    ...
    @empty
        <div class="col-span-2 bg-white rounded-2xl border border-gray-200 p-8 text-center text-gray-500">
            ...
            <p class="font-bold text-gray-700 text-sm">Belum ada data ruas jalan untuk dihitung.</p>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3.5 text-right text-xs font-bold text-gray-500 uppercase tracking-wider w-48">Nilai Akhir MOORA (Yi)</th>
                    <th scope="col" class="px-6 py-3.5 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-44">Status Prioritas</th>
                </tr>
            </thead>
                ...
                @forelse ($results as $row)
                    <tr class="hover:bg-gray-50 transition-colors {{ $row['rank'] === 1 ? 'bg-amber-50/40' : '' }}">
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            @elseif ($row['rank'] === 3)
                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-amber-100 text-amber-900 font-bold text-xs">
                                    #3
                                </span>
                            @else
