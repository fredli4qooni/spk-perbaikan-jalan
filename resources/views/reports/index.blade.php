...
<!-- Info Banner (Ringkas) -->
<div class="flex items-center gap-2.5 bg-blue-50/80 border border-blue-200/80 px-3.5 py-2.5 mb-4 rounded-xl text-xs text-blue-800 shadow-2xs">
...
<!-- Sub-bar: Total Rekapitulasi & Opsi Tampilan Per Halaman -->
<div class="flex items-center justify-between gap-2 mb-4">
    <div class="flex items-center gap-1.5">
        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/80">
            <i class="bi bi-file-earmark-check text-xs"></i>
            <span>{{ $results->total() }} Ruas Masuk Laporan</span>
        </span>
    </div>
...
<div class="grid grid-cols-2 gap-2.5 sm:gap-3.5 md:hidden">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-xs p-3.5 sm:p-4 flex flex-col justify-between space-y-3 h-full {{ $isRank1 ? 'border-amber-300 ring-2 ring-amber-200/60 bg-amber-50/25' : ($isTop3 ? 'border-purple-200/90' : 'border-gray-200') }}">
            <div class="space-y-2">
                <div class="flex items-center justify-between gap-1.5">
                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-xl font-bold text-xs flex-shrink-0 {{ $isRank1 ? 'bg-amber-400 text-amber-950 ring-1 ring-amber-300' : ($isTop3 ? 'bg-purple-100 text-brand-purple' : 'bg-gray-100 text-gray-700') }}">
                        #{{ $row['rank'] }}
                    </span>
                    @if($isRank1)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-900 border border-amber-300">
    ...
    @empty
        <div class="col-span-2 bg-white rounded-2xl border border-gray-200 p-8 text-center text-gray-500">
            ...
            <p class="font-medium text-gray-700 text-sm">Belum ada data hasil perhitungan.</p>
        </div>
    @endforelse
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Wilayah (Kec/Kel)</th>
                    <th scope="col" class="px-6 py-3.5 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider w-44">Nilai MOORA (Yi)</th>
                    <th scope="col" class="px-6 py-3.5 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider w-40">Status</th>
                </tr>
            </thead>
                ...
                @forelse ($results as $row)
                    <tr class="hover:bg-gray-50 transition-colors {{ $row['rank'] === 1 ? 'bg-amber-50/40' : '' }}">
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-semibold {{ $row['rank'] <= 3 ? 'bg-amber-400 text-amber-950 ring-2 ring-amber-100' : 'bg-gray-100 text-gray-800' }}">
                                #{{ $row['rank'] }}
                            </span>
                        </td>
