...
<div class="flex items-center justify-between gap-3 mb-5">
    ...
    <a href="{{ route('criteria.create') }}" class="inline-flex items-center justify-center rounded-xl bg-brand-purple px-3.5 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm font-semibold text-white shadow-xs hover:bg-brand-purple-hover focus:outline-none focus:ring-2 focus:ring-brand-purple transition-all flex-shrink-0">
...
<div class="grid grid-cols-2 gap-3 md:hidden">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-xs p-3.5 sm:p-4 flex flex-col justify-between space-y-3 h-full">
            ...
            <div>
                <div class="flex items-center justify-between gap-1.5 mb-2">
                    <span class="w-8 h-8 rounded-xl bg-purple-50 text-brand-purple font-bold text-xs flex items-center justify-center border border-purple-100 flex-shrink-0">
                        {{ $criterion->code }}
                    </span>
                ...
                <h3 class="font-semibold text-gray-900 text-xs sm:text-sm leading-snug line-clamp-2">{{ $criterion->name }}</h3>
    ...
    @empty
        <div class="col-span-2 bg-white rounded-2xl border border-gray-200 p-8 text-center text-gray-500">
            ...
            <p class="font-semibold text-gray-700 text-sm">Belum ada data kriteria.</p>
        </div>
    @endforelse
</div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-24"><i class="bi bi-tag"></i> Kode</th>
                    <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nama Kriteria</th>
                    <th scope="col" class="px-6 py-3.5 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider w-28">Bobot</th>
                    <th scope="col" class="px-6 py-3.5 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider w-28">Tipe</th>
                    <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-36">Satuan</th>
                    <th scope="col" class="px-6 py-3.5 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider w-32">Aksi</th>
                </tr>
            </thead>
                ...
                @forelse ($criteria as $criterion)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-purple-50 text-brand-purple font-bold text-xs border border-purple-100">
                                {{ $criterion->code }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-semibold text-gray-900">{{ $criterion->name }}</div>
                            <div class="text-xs text-gray-400 mt-0.5">{{ $criterion->description }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-normal">
                            {{ $criterion->unit ?? '-' }}
                        </td>
